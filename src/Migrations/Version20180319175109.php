 ...
 */
class Version20180319175109 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__release AS SELECT version, date, type, tar_package_md5sum, tar_package_sha1sum, tar_package_sha256sum, zip_package_md5sum, zip_package_sha1sum, zip_package_sha256sum FROM "release"');
        $this->addSql('DROP TABLE "release"');
        $this->addSql('CREATE TABLE "release" (version VARCHAR(255) NOT NULL COLLATE BINARY, date DATETIME NOT NULL, type VARCHAR(255) NOT NULL COLLATE BINARY, tar_package_md5sum VARCHAR(255) DEFAULT NULL COLLATE BINARY, tar_package_sha1sum VARCHAR(255) DEFAULT NULL COLLATE BINARY, tar_package_sha256sum VARCHAR(255) DEFAULT NULL COLLATE BINARY, zip_package_md5sum VARCHAR(255) DEFAULT NULL COLLATE BINARY, zip_package_sha1sum VARCHAR(255) DEFAULT NULL COLLATE BINARY, zip_package_sha256sum VARCHAR(255) DEFAULT NULL COLLATE BINARY, PRIMARY KEY(version), CONSTRAINT FK_9E47031DBF1CD3C3 FOREIGN KEY (version) REFERENCES major_version (version) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO "release" (version, date, type, tar_package_md5sum, tar_package_sha1sum, tar_package_sha256sum, zip_package_md5sum, zip_package_sha1sum, zip_package_sha256sum) SELECT version, date, type, tar_package_md5sum, tar_package_sha1sum, tar_package_sha256sum, zip_package_md5sum, zip_package_sha1sum, zip_package_sha256sum FROM __temp__release');
